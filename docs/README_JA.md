...
インフライトが `kcc_cwnd_min_target` まで低下するか、ラウンド境界が観測された後、少なくとも `kcc_probe_rtt_mode_ms_val`（デフォルト 200 ms）以上かつ少なくとも 1 完全ラウンドが観測された後に持続し、その後終了。cwnd は少なくとも `prior_cwnd` に復元され、ペーシングは一時的に `kcc_high_gain_val` で上書きされ、パイプを迅速に再充填する。
...
ラウンド境界は BBR（Cardwell et al. 2016）に従って検出されます：`prior_delivered` が符号なし `!before()` 比較により `next_rtt_delivered` 以上になった場合。`next_rtt_delivered` は `0` に初期化されます — 標準 BBR と同様 — そのため、最初の ACK はハンドシェイクセグメントの配信に関係なく直ちにラウンド 1 を開始します。レートサンプルの検証は `interval_us <= 0`（`== 0` ではない）を使用して BBR の正確なガードに一致させ、負の間隔を捕捉します。
...
LT BW プローブブースト（`kcc_lt_bw_probe_pct`、デフォルト 10%）：すべての PROBE_BW フェーズで pacing_gain を `1 + probe_pct/100` だけ増幅。ランプ成分：`8 RTT ごとに +1%` 増加、最大 `2 × probe_pct`。

LT BW 自動回復（`kcc_lt_restore_ratio_num/den`、デフォルト 5/4 = 1.25x）：`kcc_lt_restore_consec_acks`（デフォルト 3）回の連続 ACK で `max_bw > lt_bw × ratio` の場合、LT BW は自動的に終了し、通常の PROBE_BW プロービングが再開される。
...
| `kcc_lt_bw_probe_pct` | 10 | 0-100 | % | LT BW プローブブースト |

### LT 自動回復

| パラメータ | デフォルト | 範囲 | 説明 |
|-----------|---------|-------|-------------|
| `kcc_lt_restore_ratio_num` / `kcc_lt_restore_ratio_den` | 5 / 4 | 0-100k / 1-100k | 回復トリガー比率 |
| `kcc_lt_restore_consec_acks` | 3 | 1-31 | トリガー連続 ACK 数 |
