...
추정된 EDT에서의 in-flight가 1.0x BDP 게인의 목표 in-flight 이하가 되면 트리거됩니다. **드레인 스킵 최적화**: 칼만 필터가 수렴되고 `qdelay_avg < 동적 클린 임계값`(min_rtt_us의 10%, 500us 하한)인 경우 DRAIN 단계를 건너뛰고 조기에 PROBE_BW로 전환합니다.
...
4. `qdelay_avg > 동적 혼잡 임계값`(min_rtt_us의 25%, 500us 하한)
...
KCC가 병목 지점에서 흐름이 단독일 가능성이 높다고 감지하면(낮은 큐 지연, 낮은 지터, ECN 마크 없음, ACK 집계 없음), 자동으로 BBR 순수 모드로 전환됩니다:
...
이를 통해 KCC와 BBR 간의 단일 흐름 처리량 격차가 제거되며, 다중 흐름 시나리오에서는 KCC의 완전한 보호 루프(칼만, ECN 백오프, 이득 감쇠)가 유지됩니다.

**히스테리시스**: 진입에는 `kcc_alone_confirm_rounds`(기본값 3)개의 연속 적격 라운드가 필요합니다——다중 흐름 경쟁 중 짧은 조용한 기간 동안의 진동을 방지합니다("보수적 가속"). 퇴장: 크루즈 단계 평가 중, 어떤 단일 적격 조건이라도 실패하면 플래그가 지워집니다("공격적 제동").

**설계 트레이드오프**: 패킷 손실은 단일 흐름 비적격 기준으로 사용되지 않습니다——일부 링크(얕은 버퍼, 무선, 가상화 버스트 드롭)에는 경쟁과 무관한 고유 손실이 있습니다. 손실을 다중 흐름 경쟁과 동일시하면 단일 흐름 경로에서 진동이 발생합니다. LT BW 신호(BBR의 손실 트리거 폴리서 감지)는 단일 흐름 판단에 참여하지 않습니다.

**게인 게이팅**: 단일 흐름 평가는 크루즈 단계(`pacing_gain == BBR_UNIT`)에서만 실행됩니다. 프로브 업(1.25x)은 의도적으로 병목을 밀어냅니다——그 큐 압력은 자기 유도적이며 경쟁 신호가 아닙니다. 드레인(0.75x)은 인위적으로 큐를 억제합니다. 평가를 크루즈(정상 상태 균형)로 제한함으로써, 자기 유도적 프로브 업 압력이 더 이상 거짓 단독 모드 종료를 유발하지 않습니다.

적격 조건(라운드 경계에서 다섯 가지 모두 충족되어야 함):
0. 칼만 수렴(`sample_cnt >= kcc_kalman_min_samples`) — qdelay/jitter를 큐 신호로 신뢰
1. `qdelay_avg < 동적 클린 임계값` — 큐가 거의 비어 있음
2. `jitter_ewma < 동적 혼잡 임계값` — ACK 클록 마이크로 지터만
3. `ecn_ewma == 0` — AQM의 혼잡 마크 없음
4. `agg_state <= max` `kcc_alone_agg_state_level` 기준 (기본값 1) — 3단계 ACK 집계 엄격도:
...
| `kcc_recal_p_est_thresh` | 25,000 | 1–100,000,000 | 안전망 재보정을 위한 p_est 임계값 |
...
**이중 임계값 혼잡 게이트**: `lt_use_bw = 1`을 설정하기 전에, 지속적인 EWMA 큐 검사(`qdelay_avg > 동적 혼잡 임계값`)와 SRTT 기반 순시 큐 검사(`srtt_us − min_rtt_us > 순시 혼잡 임계값`, 기본값 5000 µs)가 모두 평가됩니다. 혼잡이 감지되면 LT BW 샘플링이 중단됩니다. SRTT 검사는 `ext` 할당 없이 작동하여 할당 실패에 대한 안전망을 제공합니다.
...
3. RTT가 `min_rtt_us + 동적 클린 임계값` 이내로 실제 전파 지연에 가까움
...
| `kcc_kalman_qboost_cdwn` | 8 | 1 | 255 | samples | Q-부스트 쿨다운 |
| `kcc_kalman_q_max` | 2000 | 1 | 100k | Q 상한 |
| `kcc_kalman_q_scale_cap` | 50 | 1 | 10000 | Q 스케일 상한 |
...
| 파라미터 | 기본값 | 범위 | 설명 |
|-----------|---------|-------|-------------|
| `kcc_kalman_outlier_jitter_mult_num/den` | 3 / 1 | 0-1000 / 1-100k | 이상치 지터 승수 |
...
| `kcc_kalman_q_est_max` | 50,000 | 1-2B | Q 추정 상한 |
| `kcc_kalman_r_est_max` | 32,000 | 1-2B | R 추정 상한 |
...
| 파라미터 | 기본값 | 범위 | 단위 | 설명 |
| `qdelay_clean_bp` | 1000 | 1-10000 | ‱ | 클린 임계값 (10% × min_rtt_us) |
| `qdelay_cong_bp` | 2500 | 1-10000 | ‱ | 혼잡 임계값 (25% × min_rtt_us) |
| `qdelay_floor_us` | 500 | 1-100k | µs | 절대 하한, 이 이하에서는 RTT 비율이 덮어쓰기됨 |
...
| `kcc_lt_loss_thresh` | 25 | 1-65535 | BBR_UNIT | 최소 손실률 |
...
| `kcc_agg_max_comp_ratio` | 50 | 0-100 | % of BDP | cwnd 보상 상한 |
...
| `kcc_recal_p_est_thresh` | 25,000 | 1-100M | 안전망 재보정을 위한 p_est 임계값 |
...
| `kcc_alone_bypass_ecn` | 0 | 0-1 | — | 단독 모드 시 ECN 백오프 건너뛰기 (1=건너뛰기, 0=활성 유지) |
