...
在途資料降至 `kcc_cwnd_min_target` 或輪邊界後，駐留至少 `kcc_probe_rtt_mode_ms_val`（預設 200 ms）且至少一輪完整輪次後退出。cwnd 恢復至 `prior_cwnd`，pacing 暫用 `kcc_high_gain_val` 覆寫以快速填充管道。

### 恢復與遺失

- TCP_CA_Loss：`full_bw` 和 `full_bw_cnt` 重置，`round_start` 設 1，`packet_conservation` 清 0。若 LT BW 未激活，注入合成遺失事件觸發 LT 取樣。
...
當 KCC 偵測到該流在瓶頸處可能為獨享狀態（低佇列延遲、低抖動、無 ECN 標記、無 ACK 聚合、無 LT 頻寬），它會自動切換至 BBR 純淨模式：
...
- LT BW（限速器偵測）條件可透過 `kcc_alone_bypass_lt_bw`（預設 1）設定——單流路徑不存在 policer，LT BW 無法合法觸發，跳過以防止假觸發導致獨狼模式退出。設為 0 恢復原始嚴格行為。

這消除了 KCC 與 BBR 之間的單流吞吐量差距，同時在多流場景下保留了 KCC 的完整保護迴路（卡爾曼、ECN 退避、增益衰減、LT 頻寬）。

**遲滯機制**：進入需要 `kcc_alone_confirm_rounds`（預設 3）個連續合格輪次——防止在多流競爭中的短暫安靜期出現振盪（「保守加速」）。退出即時生效——任何條件不滿足都將清除旗標並重置確認計數器（「激進制動」）。

合格條件（在輪次邊界上必須同時滿足全部六項）：
...
4. `lt_use_bw == 0` — 未處於限速器偵測到的速率限制模式
5. `agg_state <= max` 按 `kcc_alone_agg_state_level`（預設 1）—— 三級可配置 ACK 聚合嚴格度：
...
LT BW 探測增強（`kcc_lt_bw_probe_pct`，預設 10%）：PROBE_BW 所有相位中 pacing_gain 放大 `1 + probe_pct/100`。斜坡部分：`+1% per 8 RTTs` 持續增加，上限 `2 × probe_pct`。

LT BW 自動恢復（`kcc_lt_restore_ratio_num/den`，預設 5/4 = 1.25x）：`max_bw > lt_bw × ratio` 持續 `kcc_lt_restore_consec_acks`（預設 3）個連續 ACK 時自動退出 LT BW。
...
// 在途資料邊界（非 STARTUP：lo~hi 夾緊；STARTUP：僅 lo 下限）
...
| `kcc_inflight_low_gain_num` / `kcc_inflight_low_gain_den` | 125 / 100 | 0/1 | 100k | 在途下限（1.25x BDP） |
| `kcc_inflight_high_gain_num` / `kcc_inflight_high_gain_den` | 200 / 100 | 0/1 | 100k | 在途上限（2.0x BDP） |
...
| `kcc_lt_bw_probe_pct` | 10 | 0-100 | % | LT BW 探測增強 |

### LT 自動恢復

| 參數 | 預設 | 範圍 | 描述 |
|-----------|---------|-------|-------------|
| `kcc_lt_restore_ratio_num` / `kcc_lt_restore_ratio_den` | 5 / 4 | 0-100k / 1-100k | 恢復觸發比例 |
| `kcc_lt_restore_consec_acks` | 3 | 1-31 | 恢復觸發連續 ACK 數 |
...
| `kcc_alone_bypass_lt_bw` | 1 | 0-1 | — | 獨狼模式下跳過 LT BW 條件檢查（1=跳過，0=保持啟用） |
