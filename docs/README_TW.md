...
通用 TCP 壅塞控制模組，結合 BBRv1 狀態機與卡爾曼濾波器進行傳播延遲估計。
...
| 模式 | 值 | 行為 | 適用場景 |
|------|----|------|----------|
| FILTER | 1（預設） | 直接使用 `x_est_us` ——原始卡爾曼/滑動視窗濾波器估計值 | 通用場景：抗路徑突變，零吞吐斷崖 |
| MIN | 0 | `min(x_est_us, min_rtt_us)` ——用視窗最小值鉗制卡爾曼估計 | 保守模式：以窗口最小值鉗制卡爾曼估計，適用於 RTT 固定的環境 |
...
EDT 在途資料量 ≤ 1.0x BDP 目標時觸發。**排水跳過優化**：卡爾曼已收斂且 `qdelay_avg < 動態乾淨閾值`（10% 的 min_rtt_us，500us 下限）時跳過 DRAIN——提前轉 PROBE_BW。
...
**模型 RTT 策略**：BDP 計算使用的 RTT 估計由 `kcc_rtt_mode` 控制。FILTER 模式（預設）下，`model_rtt = x_est_us` 直接使用卡爾曼/滑動視窗估計值，不做鉗制。MIN 模式下，`model_rtt = min(x_est_us, min_rtt_us)` ——卡爾曼估計值被視窗最小值鉗制，確保 BDP 永不膨脹。通用部署推薦使用 FILTER 預設值。在路徑延遲可能突變的場景（BGP 重路由、低軌衛星切換、移動蜂窩切換）中，FILTER 模式可在數個 RTT 內完成自適應，而 MIN 模式將被鎖死在過時的 `min_rtt_us` 上。參見 [模型 RTT 策略](#模型-rtt-策略)。
...
4. `qdelay_avg > 動態擁塞閾值`（25% 的 min_rtt_us，500us 下限）
...
1. `qdelay_avg < 動態乾淨閾值`—— 佇列接近為空
2. `jitter_ewma < 動態擁塞閾值`—— 僅 ACK 時鐘微抖動
...
| `kcc_recal_p_est_thresh` | 25,000 | 1–100,000,000 | 觸發安全網重校準的 p_est 閾值 |
...
**雙閾值壅塞門控**：在設置 `lt_use_bw = 1` 之前，評估持久 EWMA 佇列檢查（`qdelay_avg > 動態擁塞閾值`）和基於 SRTT 的瞬時佇列檢查（`srtt_us − min_rtt_us > 瞬時擁塞閾值`，預設 5000 µs）。檢測到壅塞時，LT BW 取樣被中止。SRTT 檢查無需 `ext` 分配即可工作。
...
| `kcc_kalman_qboost_cdwn` | 8 | 1 | 255 | samples | Q-boost 冷卻 |
| `kcc_kalman_q_max` | 2000 | 1 | 100k | Q 上限 |
| `kcc_kalman_q_scale_cap` | 50 | 1 | 10000 | Q 縮放上限 |
...
| 參數 | 預設 | 範圍 | 描述 |
|-----------|---------|-------|-------------|
| `kcc_kalman_outlier_jitter_mult_num/den` | 3 / 1 | 0-1000 / 1-100k | 異常值抖動乘數 |
...
| `kcc_kalman_q_est_max` | 50,000 | 1-2B | Q 估計上限 |
| `kcc_kalman_r_est_max` | 32,000 | 1-2B | R 估計上限 |
...
| 參數 | 預設 | 範圍 | 單位 | 說明 |
| `qdelay_clean_bp` | 1000 | 1-10000 | ‱ | 乾淨閾值（10% × min_rtt_us） |
| `qdelay_cong_bp` | 2500 | 1-10000 | ‱ | 擁塞閾值（25% × min_rtt_us） |
| `qdelay_floor_us` | 500 | 1-100k | µs | 絕對地板，低於此時 RTT 百分比被覆蓋 |
...
| `kcc_lt_loss_thresh` | 25 | 1-65535 | BBR_UNIT | 最小遺失率 |
...
| `kcc_agg_max_comp_ratio` | 50 | 0-100 | % of BDP | cwnd 補償上限 |
...
| `kcc_recal_p_est_thresh` | 25,000 | 1-100M | 安全網重校準的卡爾曼 p_est 閾值 |
...
| `kcc_alone_bypass_ecn` | 0 | 0-1 | — | 獨狼模式下跳過 ECN 退避（1=跳過，0=保持啟用） |
...
KCC 在 `/proc/kcc/status` 提供唯讀 proc 檔案用於每連線診斷，顯示卡爾曼濾波器
內部狀態、佇列壓力和降級標誌。

**全域部分：**
- `ext_fail` — `struct kcc_ext` 分配失敗次數（>0 表示至少有一個連線正在降級執行）

**每連線欄位：** ident, min_rtt, mode, rtt_m(F=Kalman/M=BBR), p_est, samp, x_est, qdelay, jitter, ecn%, agg, alone, lt, qb_cd, rej

若 `ext_fail > 0`，請檢查 `dmesg` 中的 `KCC: ext alloc failed` 警告。
