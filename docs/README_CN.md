...
当 KCC 检测到该流在瓶颈处可能为独享状态（低队列延迟、低抖动、无 ECN 标记、无 ACK 聚合、无 LT 带宽），它会自动切换至 BBR 纯净模式：
...
- LT BW（限速器检测）条件可通过 `kcc_alone_bypass_lt_bw`（默认 1）配置——单流路径不存在 policer，LT BW 无法合法触发，跳过以防止假触发导致独狼模式退出。设为 0 恢复原始严格行为。

这消除了 KCC 与 BBR 之间的单流吞吐量差距，同时在多流场景下保留了 KCC 的完整保护环路（卡尔曼、ECN 退避、增益衰减、LT 带宽）。

**迟滞机制**：进入需要 `kcc_alone_confirm_rounds`（默认 3）个连续合格轮次——防止在多流竞争中的短暂安静期出现振荡（"保守加速"）。退出即时生效——任何条件不满足都将清除标志并重置确认计数器（"激进制动"）。

合格条件（在轮次边界上必须同时满足全部六项）：
...
4. `lt_use_bw == 0` — 未处于限速器检测到的速率限制模式
5. `agg_state <= max` 按 `kcc_alone_agg_state_level`（默认 1）—— 三级可配置 ACK 聚合严格度：
...
// 在途数据边界（非 STARTUP：lo~hi 夹紧；STARTUP：仅 lo 下限）
...
| `kcc_alone_bypass_lt_bw` | 1 | 0-1 | — | 独狼模式下跳过 LT BW 条件检查（1=跳过，0=保持启用） |
