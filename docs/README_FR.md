...
Module universel de contrôle de congestion TCP, combinant la machine d'état BBRv1 avec un filtre de Kalman pour l'estimation du délai de propagation.
...
|------|--------|--------------|-------------------|
| FILTER | 1 (défaut) | Utilisation directe de `x_est_us` — l'estimation brute du filtre de Kalman/à fenêtre glissante | Usage général : résistant aux changements de chemin, sans falaise de débit |
| MIN | 0 | `min(x_est_us, min_rtt_us)` — limiter l'estimation de Kalman par le minimum fenêtré | Mode conservateur : borner Kalman par le minimum de fenêtre, pour environnements à RTT statique |
...
Déclenché lorsque le trafic en vol estimé à EDT ≤ trafic en vol cible à gain BDP de 1,0x. **Optimisation de saut de DRAIN** : lorsque le filtre de Kalman est convergé ET `qdelay_avg < le seuil propre dynamique` (10% du min_rtt_us avec plancher 500us), la phase DRAIN est sautée — conversion précoce vers PROBE_BW.
...
**Stratégie de RTT modèle** : L'estimation RTT utilisée pour le calcul du BDP est contrôlée par `kcc_rtt_mode`. En mode FILTER (défaut), `model_rtt = x_est_us` est utilisé directement — l'estimation de Kalman/à fenêtre glissante sans limitation. En mode MIN, `model_rtt = min(x_est_us, min_rtt_us)` — l'estimation de Kalman est limitée par le minimum fenêtré pour garantir que le BDP ne gonfle jamais. La valeur par défaut FILTER est recommandée pour les déploiements universels. Dans les environnements où la latence du chemin peut changer brusquement (reroutage BGP, handover LEO, changement de cellule mobile), le mode FILTER s'adapte en quelques RTTs, tandis que le mode MIN reste bloqué sur l'ancien `min_rtt_us`. Voir [Stratégie de RTT modèle](#strat%C3%A9gie-de-rtt-mod%C3%A8le).
...
4. `qdelay_avg > le seuil de congestion dynamique` (25% du min_rtt_us avec plancher 500us)
...
Lorsqu'KCC détecte que le flux est probablement seul sur le goulot d'étranglement (faible délai de file d'attente, faible gigue, aucune marque ECN, aucune agrégation d'ACK), il passe automatiquement en mode BBR pur :
...
Cela élimine l'écart de performance en flux unique entre KCC et BBR, tout en préservant la boucle de protection complète d'KCC (Kalman, backoff ECN, décroissance de gain) pour les scénarios multi-flux.

**Hystérésis** : L'entrée nécessite `kcc_alone_confirm_rounds` (défaut 3) rounds consécutifs qualifiés — évitant les oscillations lors de brèves périodes d'accalmie dans la compétition multi-flux ("conservateur pour accélérer"). Sortie : lors de l'évaluation en phase de croisière, tout échec de qualification unique efface le drapeau ("agressif pour freiner").

**Compromis de conception** : La perte de paquets n'est PAS utilisée comme disqualifiant de flux unique — certaines liaisons (tampons peu profonds, sans fil, pertes en rafale de virtualisation) ont des pertes inhérentes sans rapport avec la concurrence. Assimiler la perte à une concurrence multi-flux provoque des oscillations sur les chemins à flux unique. Le signal LT BW (détection de policer déclenchée par les pertes de BBR) ne participe pas au jugement de flux unique.

**Gain gating** : l'évaluation de flux unique s'exécute uniquement pendant la phase de croisière (`pacing_gain == BBR_UNIT`). La sonde montante (1,25x) pousse intentionnellement contre le goulot d'étranglement — sa pression de file d'attente est auto-induite et non un signal de concurrence. Le drain (0,75x) supprime artificiellement la file d'attente. En limitant l'évaluation à la croisière (l'équilibre stationnaire), la pression de sonde montante auto-induite ne provoque plus de fausses sorties du mode seul.

Conditions de qualification (les cinq doivent être satisfaites à une limite de tour) :
0. Kalman convergé (`sample_cnt >= kcc_kalman_min_samples`) — faire confiance à qdelay/jitter comme signaux de file
1. `qdelay_avg < le seuil propre dynamique` — file presque vide
2. `jitter_ewma < le seuil de congestion dynamique` — micro-gigue d'horloge ACK uniquement
3. `ecn_ewma == 0` — aucune marque de congestion d'AQM
4. `agg_state <= max` selon `kcc_alone_agg_state_level` (par défaut 1) — trois niveaux de rigueur d'agrégation ACK : 0 = IDLE seulement (le plus strict, zéro agrégation), 1 = ≤ SUSPECTED (par défaut, permet l'agrégation transitoire), 2 = ≤ CONFIRMED (le plus permissif, bloque uniquement l'agrégation persistante)
...
| `kcc_recal_p_est_thresh` | 25000 | 1–100 000 000 | Seuil p_est pour le filet de sécurité de recalibrage |
...
**Porte de congestion à double seuil** : Avant de définir `lt_use_bw = 1`, une vérification de file EWMA persistante (`qdelay_avg > le seuil de congestion dynamique`) ET une vérification de file instantanée basée sur SRTT (`srtt_us − min_rtt_us > le seuil de congestion instantané`, par défaut 5000 µs) sont évaluées. Lorsqu'une congestion est détectée, l'échantillonnage LT BW est abandonné. La vérification SRTT fonctionne sans allocation `ext`, fournissant un filet de sécurité contre les échecs d'allocation.
...
3. RTT dans `min_rtt_us + le seuil propre dynamique` du délai de propagation réel
...
1. Bloque si le délai de file > `le seuil de congestion dynamique`
...
| `kcc_kalman_qboost_cdwn` | 8 | 1 | 255 | samples | Refroidissement Q-boost |
| `kcc_kalman_q_max` | 2000 | 1 | 100k | Plafond Q |
| `kcc_kalman_q_scale_cap` | 50 | 1 | 10000 | Limite d'échelle Q |
...
|-----------|------------|-------|-------------|
| `kcc_kalman_outlier_jitter_mult_num/den` | 3 / 1 | 0-1000 / 1-100k | Multiplicateur de gigue pour valeurs aberrantes |
...
| `kcc_kalman_q_est_max` | 50000 | 1-2 Mds | Borne supérieure estimation Q |
| `kcc_kalman_r_est_max` | 32000 | 1-2 Mds | Borne supérieure estimation R |
...
### Seuils Dynamiques de Délai de File d'Attente

| Paramètre | Défaut | Plage | Unité | Description |
|-----------|--------|-------|-------|-------------|
| `kcc_qdelay_clean_bp` | 1000 | 1-10000 | ‱ | Seuil propre (10 % du min_rtt_us) |
| `kcc_qdelay_cong_bp` | 2500 | 1-10000 | ‱ | Seuil de congestion (25 % du min_rtt_us) |
| `kcc_qdelay_floor_us` | 500 | 1-100k | µs | Plancher absolu pour le remplacement du pourcentage RTT |
...
| `kcc_lt_loss_thresh` | 25 | 1-65535 | BBR_UNIT | Ratio de perte min |
...
| `kcc_agg_max_comp_ratio` | 50 | 0-100 | % du BDP | Plafond de compensation cwnd |
...
| `kcc_recal_p_est_thresh` | 25000 | 1-100M | Seuil p_est pour le filet de sécurité de recalibrage |
...
| `kcc_alone_bypass_ecn` | 0 | 0-1 | — | Ignorer le backoff ECN en mode solo (1=ignorer, 0=actif) |
...
KCC expose `/proc/kcc/status` pour le diagnostic par connexion, affichant
l'état interne du filtre de Kalman, la pression de file d'attente et les drapeaux de dégradation.

**Section globale :**
- `ext_fail` — échecs d'allocation de `struct kcc_ext` (>0 = ≥1 connexion dégradée)

**Colonnes par connexion :** ident, min_rtt, mode, rtt_m(F=Kalman/M=BBR), p_est, samp, x_est, qdelay, jitter, ecn%, agg, alone, lt, qb_cd, rej

Si `ext_fail > 0`, vérifiez `dmesg` pour les avertissements `KCC: ext alloc failed`.
