...
LT-BW-Sondenverstärkung (`kcc_lt_bw_probe_pct`, Standard 10%): verstärkt `pacing_gain` um `1 + probe_pct/100` über alle PROBE_BW-Phasen. Rampenkomponente: `+1% pro 8 RTTs` Anstieg, gedeckelt bei `2 × probe_pct`.

LT-BW-Auto-Wiederherstellung (`kcc_lt_restore_ratio_num/den`, Standard 5/4 = 1,25x): wenn `max_bw > lt_bw × ratio` für `kcc_lt_restore_consec_acks` (Standard 3) aufeinanderfolgende ACKs, wird LT BW automatisch beendet und die normale PROBE_BW-Erkundung wird fortgesetzt.
...
| `kcc_inflight_low_gain_num` / `kcc_inflight_low_gain_den` | 125 / 100 | 0/1 | 100k | Inflight-Untergrenze (1,25x BDP) |
...
| `kcc_lt_bw_probe_pct` | 10 | 0-100 | % | LT-BW-Sondenverstärkung |

### LT-Auto-Wiederherstellung

| Parameter | Standard | Bereich | Beschreibung |
|-----------|----------|---------|-------------|
| `kcc_lt_restore_ratio_num` / `kcc_lt_restore_ratio_den` | 5 / 4 | 0-100k / 1-100k | Wiederherstellungsauslöseverhältnis |
| `kcc_lt_restore_consec_acks` | 3 | 1-31 | Auslöse-Anzahl aufeinanderfolgender ACKs |
