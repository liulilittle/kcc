#!/usr/bin/env python3
"""MINIMAL GEODESIC: x_est=min(x_est+,z) up/down. Confirm+qdelay->mr. BDP=mr."""
...
class K:
    ...
    def step(self, rtt, ql=0):
        ...
        if self.conf >= 4 or self.conf_slow >= 5:
            ...
            self.conf = 0
            self.conf_slow = 0
        self.q = self.q * 0.875 + max(0, ar - self.x // SCALE) * 0.125
...
NS = 30
EXP_RTT = {25, 50, 100, 200}
EXP_SP = {5, 10}
print("=" * 60)
print(f"MINIMAL GEODESIC {len(KEY)}x{len(STEPS)}x{NS}")
print("=" * 60)
...
for T in KEY:
    ...
    for sp in STEPS:
        ...
        if Tn == T:
            continue
        missed = 0
        expected = T in EXP_RTT and sp in EXP_SP
...
P("Path increase: ok") if failures == 0 else None
...
dt = time.time() - t0
P(
    f"\n{int(dt)}s. Verdict: Geodesic: G1,G2,G3 only. p_est,Q,R,outlier,pos_skip,drift removed.",
)
